...
// Mutable number-tree lookup. Bound recursion and detect cycles in malformed
// trees.
...
void CPDF_RedactionMarkSanitizer::RemoveUnusedProperties(
  ...
  auto private_resources = ToDictionary(resources->Clone());
  auto private_properties = ToDictionary(properties->Clone());
...
void CPDF_RedactionMarkSanitizer::RemoveUnusedPageTreeProperties() {
  ...
  for (int i = 0; i < doc->GetPageCount(); ++i) {
    ...
    // Inspect other pages without changing their content.
